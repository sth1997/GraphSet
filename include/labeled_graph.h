...
public:
    ...
    int fsm(int max_edge, long long _min_support, int thread_count, double *time_out = nullptr); // return the number of frequent labeled patterns with max_edge edges
    int fsm_pattern_matching(int job_start, int job_end, const Schedule_IEP &schedule, const char *all_p_label, std::vector<std::vector<int> > &automorphisms, unsigned int* is_frequent, unsigned int& pattern_is_frequent_index, int max_edge, int min_support, int thread_count) const;
    int fsm_vertex(int max_edge, long long _min_support, int thread_count, double *time_out = nullptr); 
    int fsm_pattern_matching_vertex(int job_id, const Schedule_IEP &schedule, const char *p_label, std::vector<std::vector<int> > &automorphisms, unsigned int* is_frequent, unsigned int& pattern_is_frequent_index,  int max_edge, int min_support, int thread_count) const;
private:
