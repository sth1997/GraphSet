...
public:
    ...
    __device__ void build_vertex_set(const GPUSchedule* schedule, const GPUVertexSet* vertex_set, GPUVertexSet& tmp_set, uint32_t* input_data, uint32_t input_size, int prefix_id)
    {
        ...
        else
        {
            ...
            if(only_need_size) {
                ...
                intersection2(this->data, tmp_set.get_data_ptr(),
                    vertex_set[father_id].get_data_ptr(), input_data, vertex_set[father_id].get_size(), input_size, &this->size);
            }
        }
    }
 ...
 */
__device__ void intersection2(uint32_t *out, uint32_t *tmp, const uint32_t *lbases, const uint32_t *rbases,
    uint32_t ln, uint32_t rn, uint32_t* p_tmp_size)
{
    // make sure ln <= rn
    if (ln > rn) {
        swap(ln, rn);
    ...
    // auto intersection_size = binary_search_intersection(out, tmp, lbases, rbases, ln, rn);
    // auto intersection_size = serial_merge_intersection(out, tmp, lbases, rbases, ln, rn);
    auto intersection_size = parallel_merge_intersection(out, tmp, lbases, rbases, ln, rn);
    ...
    GPUVertexSet& tmp_set, unsigned long long& local_ans, int depth, uint32_t *edge, uint32_t *vertex)
{
    ...
    for (int i = 0; i < loop_size; ++i)
    {
        ...
        {
            vertex_set[prefix_id].build_vertex_set(schedule, vertex_set, tmp_set, &edge[l], r - l, prefix_id);
