...
public:
    ...
    __device__ void set_size(int size) { this->size = size;}
...
// result1 = inter(set1,set3).size
// result2 = inter(set2,set4).size
__device__ void two_intersection_size(int &result1, int &result2, uint32_t *set1, uint32_t size1, uint32_t *set2, uint32_t size2, uint32_t *set3, uint32_t size3, uint32_t *set4, uint32_t size4) {
    if(size1 > size3) {
        swap(set1, set3);
        swap(size1, size3);
    }
    ...
    __shared__ int block_ret[WARPS_PER_BLOCK * 2];

    int wid = threadIdx.x / THREADS_PER_WARP;
    int lid = threadIdx.x % THREADS_PER_WARP;
    int &ret1 = block_ret[wid * 2];
    int &ret2 = block_ret[wid * 2 + 1];
    if (lid == 0) {
        ret1 = ret2 = 0;
    }
    __threadfence_block();
    
    int total_task = size1 + size2;
    ...
    while (done1 < total_task)
    {
        if (lid + done1 < total_task)
        {
            if(lid + done1 < size1) {
                ...
                l = 0;
                r = size4 - 1;
                val = set2[lid + done1 - size1];
                set_now = set4;
            }
            //int l = 0, r = (lid + done1 < size1 ? size3 - 1 : size4 - 1);
            //uint32_t val = (lid + done1 < size1 ? set1[lid + done1] : set2[lid + done1 - size1]);
            //uint32_t* set_now = (lid + done1 < size1 ? set3 : set4);
            //考虑之后换一下二分查找的写法，比如改为l < r，然后把mid的判断从循环里去掉，放到循环外(即最后l==r的时候)
            while (l <= r)
            {
                int mid = (l + r) >> 1;
    ...
    GPUVertexSet& tmp_set, unsigned long long& local_ans, uint32_t *edge, uint32_t *vertex)
{
    ...
    for (int i = 0; i < loop_size; ++i)
    {
        ...
        int s1,s2;
        two_intersection_size(s1,s2,vertex_set[2].get_data_ptr(), vertex_set[2].get_size(), vertex_set[1].get_data_ptr(), vertex_set[1].get_size(), &edge[l], r - l, &edge[l], r - l);
        /*
        vertex_set[3].build_vertex_set(schedule, vertex_set, &edge[l], r - l, 3);
        if(vertex_set[3].get_size() == 1) continue;
        vertex_set[4].build_vertex_set(schedule, vertex_set, &edge[l], r - l, 4);
        */
        if(s1 == 1) continue;
        vertex_set[3].set_size(s1);
        vertex_set[4].set_size(s2);
