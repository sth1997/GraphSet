...
__device__ void intersection2(uint32_t *tmp, const uint32_t *lbases, const uint32_t *rbases, uint32_t ln, uint32_t rn, const uint32_t *lbases_stack, const uint32_t *rbases_stack, bool lstack_flag, bool rstack_flag, uint32_t* p_tmp_size);
static __device__ uint32_t do_intersection(uint32_t*, const uint32_t*, const uint32_t*, uint32_t, uint32_t);
static __device__ uint32_t do_stack_intersection(uint32_t*, const uint32_t*, const uint32_t*, uint32_t, uint32_t, const uint32_t*);
...
public:
    ...
    __device__ uint32_t* get_stack_data_ptr() const { return stack_data;}
    __device__ bool get_stack_data_exist() const { return stack_data_exist;}
    ...
    __device__ void init(uint32_t input_size, uint32_t* input_data, uint32_t* input_stack_data)
    {
        size = input_size;
        data = input_data; //之后如果把所有prefix放到shared memory，由于input data在global memory上（因为是原图的边集），所以改成memcpy
        stack_data = input_stack_data;
        stack_data_exist = true;
    }
    ...
    __device__ void build_vertex_set(const GPUSchedule* schedule, const GPUVertexSet* vertex_set, uint32_t* input_data, uint32_t* input_stack_data, uint32_t input_size, int prefix_id)
    {
        int father_id = schedule->get_father_prefix_id(prefix_id);
        if (father_id == -1)
        {
            if (threadIdx.x % THREADS_PER_WARP == 0)
                init(input_size, input_data, input_stack_data);
            __threadfence_block();
        }
        else
        {
            intersection2(this->data, vertex_set[father_id].get_data_ptr(), input_data, vertex_set[father_id].get_size(), input_size, vertex_set[father_id].get_stack_data_ptr(), input_stack_data, vertex_set[father_id].get_stack_data_exist(), true, &this->size);
        }
    }
...
/**
 * search-based intersection with data rearrangement
 * 
 * returns the size of the intersection set
 * 
 * @note：a和b并不是地位相等的。如果要进行in-place操作，请把输入放在a而不是b。
 * @todo：shared memory缓存优化
 */
__device__ uint32_t do_stack_intersection(uint32_t* out, const uint32_t* a, const uint32_t* b, uint32_t na, uint32_t nb, const uint32_t* stack_b)
{
    __shared__ uint32_t block_out_offset[THREADS_PER_BLOCK];
    __shared__ uint32_t block_out_size[WARPS_PER_BLOCK];
...
__device__ void intersection2(uint32_t *tmp, const uint32_t *lbases, const uint32_t *rbases, uint32_t ln, uint32_t rn, const uint32_t *lbases_stack, const uint32_t *rbases_stack, bool lstack_flag, bool rstack_flag, uint32_t* p_tmp_size)
{
    ...
    if (ln > rn) {
        swap(ln, rn);
        swap(lbases, rbases);
        swap(lbases_stack, rbases_stack);
        swap(lstack_flag, rstack_flag);
    }
    /**
    ...
    if(rstack_flag) {
        /*
        uint32_t tmp_size = do_stack_intersection(tmp, lbases, rbases, ln, rn, rbases_stack);
        intersection_size = do_intersection(tmp, lbases, rbases, ln, rn);
        if(tmp_size != intersection_size)
            printf("error\n");
        */

        intersection_size = do_stack_intersection(tmp, lbases, rbases, ln, rn, rbases_stack);
    }
    else {
        intersection_size = do_intersection(tmp, lbases, rbases, ln, rn);
    }
...
__device__ void GPU_pattern_matching_func(const GPUSchedule* schedule, GPUVertexSet* vertex_set, GPUVertexSet& subtraction_set,
    GPUVertexSet& tmp_set, unsigned long long& local_ans, uint32_t *edge, uint32_t *stack_edge, uint32_t *vertex)
{
    int loop_set_prefix_id = schedule->get_loop_set_prefix_id(depth);
    int loop_size = vertex_set[loop_set_prefix_id].get_size();
    if (loop_size <= 0)
    ...
    for (int i = 0; i < loop_size; ++i)
    {
        ...
        for (int prefix_id = schedule->get_last(depth); prefix_id != -1; prefix_id = schedule->get_next(prefix_id))
        {
            vertex_set[prefix_id].build_vertex_set(schedule, vertex_set, &edge[l], &stack_edge[l], r - l, prefix_id);
            if (vertex_set[prefix_id].get_size() == 0) {
                is_zero = true;
                break;
            }
        ...
        GPU_pattern_matching_func<depth + 1>(schedule, vertex_set, subtraction_set, tmp_set, local_ans, edge, stack_edge, vertex);
        if (depth + 1 != MAX_DEPTH) {
            if (threadIdx.x % THREADS_PER_WARP == 0)
                subtraction_set.pop_back();
            __threadfence_block();
...
__global__ void gpu_pattern_matching(uint32_t edge_num, uint32_t buffer_size, uint32_t *edge_from, uint32_t *edge, uint32_t *vertex, uint32_t *tmp, const GPUSchedule* schedule, uint32_t* stack_edge) {
    ...
    while (true) {
       ...
       // for edge in E
        ...
        GPU_pattern_matching_func<2>(schedule, vertex_set, subtraction_set, tmp_set, local_sum, edge, stack_edge, vertex);
...
void pattern_matching_init(Graph *g, const Schedule& schedule) {
    ...
    if(!g->stack_edge_exist) {
        printf("Error: You must use stack edge in this program.\n");
        return;
    }
    ...
    uint32_t *stack_edge = new uint32_t[g->e_cnt];

    for(uint32_t i = 0;i < g->e_cnt; ++i) edge[i] = g->edge[i];
    for(uint32_t i = 0;i <= g->v_cnt; ++i) vertex[i] = g->vertex[i];
    for(uint32_t i = 0;i < g->e_cnt; ++i) stack_edge[i] = g->stack_edge[i];
    ...
    gpuErrchk( cudaMalloc((void**)&dev_stack_edge, size_edge));
    ...
    gpuErrchk( cudaMemcpy(dev_stack_edge, stack_edge, size_edge, cudaMemcpyHostToDevice));
    ...
    gpu_pattern_matching<<<num_blocks, THREADS_PER_BLOCK, block_shmem_size>>>
        (edge_num, buffer_size, dev_edge_from, dev_edge, dev_vertex, dev_tmp, dev_schedule, dev_stack_edge);
    ...
    gpuErrchk(cudaFree(dev_stack_edge));
    ...
    delete[] edge, edge_from, vertex, stack_edge;
}
...
int main(int argc,char *argv[]) {
    ...
    g->edge_rearrangement(thread_count);
    
    t2 = system_clock::now();
    
    load_time = std::chrono::duration_cast<std::chrono::microseconds>(t2 - t1);
    printf("Data rearrangement time: %g seconds\n", load_time.count() / 1.0e6);
    fflush(stdout);
