 ...
 */
 ...
 template <int depth>
 __device__ void GPU_pattern_matching_func(const GPUSchedule* schedule, GPUVertexSet* vertex_set, unsigned long long& local_ans, uint32_t *edge, uint64_t *vertex)
 {
     ...
     template <>
 __device__ void GPU_pattern_matching_func<MAX_DEPTH>(const GPUSchedule* schedule, GPUVertexSet* vertex_set, unsigned long long& local_ans, uint32_t *edge, uint64_t *vertex)
 {
     // assert(false);
 }
  ...
  */
 __global__ void gpu_pattern_matching(uint32_t edge_num, uint32_t buffer_size, uint32_t *edge_from, uint32_t *edge, uint64_t *vertex, uint32_t *tmp, const GPUSchedule* schedule) {
 ...
 void pattern_matching_init(Graph *g, const Schedule_IEP& schedule_iep) {
     ...
     size_t size_vertex = (g->v_cnt + 1) * sizeof(uint64_t);
