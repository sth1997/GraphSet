...
class GPUVertexSet {
  public:
    ...
    __device__ void build_vertex_set(const GPUSchedule *schedule, const GPUVertexSet *vertex_set, uint32_t *input_data, uint32_t input_size,
                                     int prefix_id) {
...
template <int depth>
__device__ bool GPU_pattern_matching_func(const GPUSchedule *schedule, GPUVertexSet *vertex_set, GPUVertexSet &subtraction_set, uint32_t *edge,
                                          uint32_t *labeled_vertex, const char *p_label, GPUBitVector *fsm_set, int l_cnt) {
...
__device__ bool GPU_pattern_matching_func<MAX_DEPTH>(const GPUSchedule *schedule, GPUVertexSet *vertex_set, GPUVertexSet &subtraction_set,
                                                     uint32_t *edge, uint32_t *labeled_vertex, const char *p_label, GPUBitVector *fsm_set,
                                                     int l_cnt) {
    assert(false);
    // return false;
}
 ...
 */
__global__ void gpu_pattern_matching(int device_id, uint32_t job_num, uint32_t v_cnt, uint32_t buffer_size, uint32_t *edge, uint32_t *labeled_vertex,
                                     int *v_label, uint32_t *tmp, const GPUSchedule *schedule, char *all_p_label, GPUBitVector *global_fsm_set,
                                     int *automorphisms, unsigned int *is_frequent, unsigned int *label_start_idx, int automorphisms_cnt,
                                     long long min_support, unsigned int pattern_is_frequent_index, int l_cnt, int *info) {
...
unsigned int *is_frequent;               // bit vectord
char *all_p_label, *tmp_p_label;
long long fsm_cnt = 0;
...
struct FSM_Device {
    ...
    int max_edge, min_support;

    // in fsm_init() function,
    ...
    FSM_Device(int _dev_id, const LabeledGraph *_g, int _max_edge, int _min_support) {
        dev_id = _dev_id, g = _g, max_edge = _max_edge, min_support = _min_support;
        ...
        // size_t size_tmp = VertexSet::max_intersection_size * sizeof(uint32_t) * num_total_warps * (max_total_prefix_num + 2); //prefix +
        // subtraction + tmp
        ...
        gpuErrchk(cudaMallocManaged((void **)&dev_fsm_set,
                                    sizeof(GPUBitVector) * num_total_warps *
                                        (max_edge + 1))); //每个点一个fsm_set，一个pattern最多max_edge+1个点，每个warp负责一个不同的labeled pattern
...
// 实际上在这里并行，我们会调用很多次核函数，让核函数在不同的 device 上面运作
long long pattern_matching_init(const LabeledGraph *g, const Schedule_IEP &schedule, const std::vector<std::vector<int>> &automorphisms,
                                unsigned int pattern_is_frequent_index, int max_edge, int job_num, long long min_support) {
...
#pragma omp parallel for
    ForallDevice(i, gpuErrchk(cudaMemcpyToSymbol(dev_sum, &sum, sizeof(sum)));)
...
#pragma omp parallel for
    ForallDevice(i, gpuErrchk(cudaMalloc((void **)&(dev[i]->dev_automorphisms), sizeof(int) * schedule.get_size() * automorphisms.size()));
                 gpuErrchk(cudaMemcpy(dev[i]->dev_automorphisms, host_automorphisms, sizeof(int) * schedule.get_size() * automorphisms.size(),
...
#pragma omp parallel for
        ForallDevice(
            ...
            // dev[i]->dev_schedule->transform_in_exclusion_optimize_group_val(schedule);
            ...
            //     gpuErrchk( cudaMallocManaged((void**)&dev[i]->dev_schedule->in_exclusion_optimize_vertex_id, sizeof(int) *
            //     in_exclusion_optimize_vertex_id_size)); gpuErrchk( cudaMemcpy(dev[i]->dev_schedule->in_exclusion_optimize_vertex_id,
            //     in_exclusion_optimize_vertex_id, sizeof(int) * in_exclusion_optimize_vertex_id_size, cudaMemcpyHostToDevice));

            //     gpuErrchk( cudaMallocManaged((void**)&dev[i]->dev_schedule->in_exclusion_optimize_vertex_flag, sizeof(bool) *
            //     in_exclusion_optimize_vertex_id_size)); gpuErrchk( cudaMemcpy(dev[i]->dev_schedule->in_exclusion_optimize_vertex_flag,
            //     in_exclusion_optimize_vertex_flag, sizeof(bool) * in_exclusion_optimize_vertex_id_size, cudaMemcpyHostToDevice));

            //     gpuErrchk( cudaMallocManaged((void**)&dev[i]->dev_schedule->in_exclusion_optimize_vertex_coef, sizeof(int) *
            //     in_exclusion_optimize_vertex_id_size)); gpuErrchk( cudaMemcpy(dev[i]->dev_schedule->in_exclusion_optimize_vertex_coef,
            //     in_exclusion_optimize_vertex_coef, sizeof(int) * in_exclusion_optimize_vertex_id_size, cudaMemcpyHostToDevice));
            // }

            // if (in_exclusion_optimize_array_size > 0)
            // {
            //     gpuErrchk( cudaMallocManaged((void**)&dev[i]->dev_schedule->in_exclusion_optimize_coef, sizeof(int) *
            //     in_exclusion_optimize_array_size)); gpuErrchk( cudaMemcpy(dev[i]->dev_schedule->in_exclusion_optimize_coef,
            //     in_exclusion_optimize_coef, sizeof(int) * in_exclusion_optimize_array_size, cudaMemcpyHostToDevice));

            //     gpuErrchk( cudaMallocManaged((void**)&dev[i]->dev_schedule->in_exclusion_optimize_flag, sizeof(bool) *
            //     in_exclusion_optimize_array_size)); gpuErrchk( cudaMemcpy(dev[i]->dev_schedule->in_exclusion_optimize_flag,
            //     in_exclusion_optimize_flag, sizeof(bool) * in_exclusion_optimize_array_size, cudaMemcpyHostToDevice));

            //     gpuErrchk( cudaMallocManaged((void**)&dev[i]->dev_schedule->in_exclusion_optimize_ans_pos, sizeof(int) *
            //     in_exclusion_optimize_array_size)); gpuErrchk( cudaMemcpy(dev[i]->dev_schedule->in_exclusion_optimize_ans_pos,
            //     in_exclusion_optimize_ans_pos, sizeof(int) * in_exclusion_optimize_array_size, cudaMemcpyHostToDevice));
            // }

            // gpuErrchk( cudaMallocManaged((void**)&dev[i]->dev_schedule->adj_mat, sizeof(int) * schedule_size * schedule_size));
            // gpuErrchk( cudaMemcpy(dev[i]->dev_schedule->adj_mat, schedule.get_adj_mat_ptr(), sizeof(int) * schedule_size * schedule_size,
            // cudaMemcpyHostToDevice));

            gpuErrchk(cudaMallocManaged((void **)&dev[i]->dev_schedule->father_prefix_id, sizeof(int) * max_prefix_num)); gpuErrchk(cudaMemcpy(
                dev[i]->dev_schedule->father_prefix_id, schedule.get_father_prefix_id_ptr(), sizeof(int) * max_prefix_num, cudaMemcpyHostToDevice));

            gpuErrchk(cudaMallocManaged((void **)&dev[i]->dev_schedule->last, sizeof(int) * schedule_size));
            gpuErrchk(cudaMemcpy(dev[i]->dev_schedule->last, schedule.get_last_ptr(), sizeof(int) * schedule_size, cudaMemcpyHostToDevice));

            gpuErrchk(cudaMallocManaged((void **)&dev[i]->dev_schedule->next, sizeof(int) * max_prefix_num));
            gpuErrchk(cudaMemcpy(dev[i]->dev_schedule->next, schedule.get_next_ptr(), sizeof(int) * max_prefix_num, cudaMemcpyHostToDevice));

            // gpuErrchk( cudaMallocManaged((void**)&dev[i]->dev_schedule->only_need_size, sizeof(bool) * max_prefix_num));
            // gpuErrchk( cudaMemcpy(dev[i]->dev_schedule->only_need_size, only_need_size, sizeof(bool) * max_prefix_num, cudaMemcpyHostToDevice));
            // TODO: 使用break size
            // gpuErrchk( cudaMallocManaged((void**)&dev[i]->dev_schedule->break_size, sizeof(int) * max_prefix_num));
            // gpuErrchk( cudaMemcpy(dev[i]->dev_schedule->break_size, schedule.get_break_size_ptr(), sizeof(int) * max_prefix_num,
            // cudaMemcpyHostToDevice));

            gpuErrchk(cudaMallocManaged((void **)&dev[i]->dev_schedule->loop_set_prefix_id, sizeof(int) * schedule_size));
            gpuErrchk(cudaMemcpy(dev[i]->dev_schedule->loop_set_prefix_id, schedule.get_loop_set_prefix_id_ptr(), sizeof(int) * schedule_size,
                                 cudaMemcpyHostToDevice));

            gpuErrchk(cudaMallocManaged((void **)&dev[i]->dev_schedule->prefix_target, sizeof(int) * max_prefix_num)); gpuErrchk(cudaMemcpy(
                dev[i]->dev_schedule->prefix_target, schedule.get_prefix_target_ptr(), sizeof(int) * max_prefix_num, cudaMemcpyHostToDevice));

            // gpuErrchk( cudaMallocManaged((void**)&dev[i]->dev_schedule->restrict_last, sizeof(int) * schedule_size));
            // gpuErrchk( cudaMemcpy(dev[i]->dev_schedule->restrict_last, schedule.get_restrict_last_ptr(), sizeof(int) * schedule_size,
            // cudaMemcpyHostToDevice));

            // gpuErrchk( cudaMallocManaged((void**)&dev[i]->dev_schedule->restrict_next, sizeof(int) * max_prefix_num));
            // gpuErrchk( cudaMemcpy(dev[i]->dev_schedule->restrict_next, schedule.get_restrict_next_ptr(), sizeof(int) * max_prefix_num,
            // cudaMemcpyHostToDevice));

            // gpuErrchk( cudaMallocManaged((void**)&dev[i]->dev_schedule->restrict_index, sizeof(int) * max_prefix_num));
            // gpuErrchk( cudaMemcpy(dev[i]->dev_schedule->restrict_index, schedule.get_restrict_index_ptr(), sizeof(int) * max_prefix_num,
            // cudaMemcpyHostToDevice));

            // dev[i]->dev_schedule->in_exclusion_optimize_array_size = in_exclusion_optimize_array_size;
            // dev[i]->dev_schedule->in_exclusion_optimize_vertex_id_size = in_exclusion_optimize_vertex_id_size;
            dev[i]->dev_schedule->size = schedule.get_size();
            dev[i]->dev_schedule->total_prefix_num = schedule.get_total_prefix_num();
            // dev[i]->dev_schedule->basic_prefix_num = schedule.get_basic_prefix_num();
            // dev[i]->dev_schedule->total_restrict_num = schedule.get_total_restrict_num();
            // dev[i]->dev_schedule->in_exclusion_optimize_num = schedule.get_in_exclusion_optimize_num();
            // dev[i]->dev_schedule->k_val = schedule.get_k_val();

            ) printf("schedule.prefix_num: %d\n", schedule.get_total_prefix_num());
    // printf("shared memory for vertex set per block: %ld bytes\n",
    //     (schedule.get_total_prefix_num() + 2) * WARPS_PER_BLOCK * sizeof(GPUVertexSet) + in_exclusion_optimize_vertex_id_size * WARPS_PER_BLOCK *
    //     sizeof(int));
    ...
    // in_exclusion_optimize_vertex_id_size * WARPS_PER_BLOCK * sizeof(int);
    uint32_t block_shmem_size = (schedule.get_total_prefix_num() + 2) * WARPS_PER_BLOCK * sizeof(GPUVertexSet) +
                                (max_edge + 1) * WARPS_PER_BLOCK * sizeof(char); // max_edge + 1是指一个pattern最多这么多点，用于存储p_label

#pragma omp parallel for
    ForallDevice(i,
                 // dev[i]->dev_schedule->ans_array_offset = block_shmem_size - in_exclusion_optimize_vertex_id_size * WARPS_PER_BLOCK * sizeof(int);
                 dev[i]->dev_schedule->p_label_offset = block_shmem_size - (max_edge + 1) * WARPS_PER_BLOCK * sizeof(char);
                 dev[i]->dev_schedule->max_edge = max_edge;
...
#pragma omp parallel for
        ...
        gpuErrchk(cudaMemcpyToSymbol(dev_sum, &tmpsum, sizeof(tmpsum)));
        ...
        gpu_pattern_matching<<<num_blocks, THREADS_PER_BLOCK, block_shmem_size>>>(
            i, job_num, g->v_cnt, buffer_size, dev[i]->dev_edge, dev[i]->dev_labeled_vertex, dev[i]->dev_v_label, dev[i]->dev_tmp,
            dev[i]->dev_schedule, dev[i]->dev_all_p_label, dev[i]->dev_fsm_set, dev[i]->dev_automorphisms, dev[i]->dev_is_frequent,
            dev[i]->dev_label_start_idx, automorphisms.size(), min_support, pattern_is_frequent_index, g->l_cnt, dev[i]->info);
...
#pragma omp parallel for
    ForallDevice(i,
                 // gpuErrchk(cudaFree(dev[i]->dev_schedule->adj_mat));
                 gpuErrchk(cudaFree(dev[i]->dev_schedule->father_prefix_id));
                 gpuErrchk(cudaFree(dev[i]->dev_schedule->last)); gpuErrchk(cudaFree(dev[i]->dev_schedule->next));
                 gpuErrchk(cudaFree(dev[i]->dev_schedule->loop_set_prefix_id)); gpuErrchk(cudaFree(dev[i]->dev_schedule->prefix_target));
                 // gpuErrchk(cudaFree(dev[i]->dev_schedule->restrict_last));
                 // gpuErrchk(cudaFree(dev[i]->dev_schedule->restrict_next));
                 // gpuErrchk(cudaFree(dev[i]->dev_schedule->restrict_index));

                 // gpuErrchk(cudaFree(dev[i]->dev_schedule->in_exclusion_optimize_vertex_id));
                 // gpuErrchk(cudaFree(dev[i]->dev_schedule->in_exclusion_optimize_coef));
                 // gpuErrchk(cudaFree(dev[i]->dev_schedule->in_exclusion_optimize_flag));

                 gpuErrchk(cudaFree(dev[i]->dev_schedule));)
...
#pragma omp parallel for
    ForallDevice(i, dev[i] = new FSM_Device(i, g, max_edge, min_support);)
    ...
    for (int i = 1; i < schedules_num; i++) {
        ...
        g->traverse_all_labeled_patterns(schedules, all_p_label, tmp_p_label, mapping_start_idx, mappings, pattern_is_frequent_index, is_frequent, i,
                                         0, mapping_start_idx_pos, all_p_label_idx);
        int job_num = all_p_label_idx / schedules[i].get_size();

// from here goes to pattern matching
#pragma omp parallel for
        ForallDevice(j, gpuErrchk(cudaMemcpy(dev[j]->dev_all_p_label, all_p_label, all_p_label_idx * sizeof(char),
                                             cudaMemcpyHostToDevice));) long long tmp_fms_cnt =
            pattern_matching_init(g, schedules[i], automorphisms, pattern_is_frequent_index[i], max_edge, job_num, min_support);
        fsm_cnt += tmp_fms_cnt;
...
#pragma omp parallel for
        ForallDevice(
            j, gpuErrchk(cudaMemcpy(tmp_is_frequent[j], &dev[j]->dev_is_frequent[is_frequent_index], is_frequent_size, cudaMemcpyDeviceToHost)););
...
#pragma omp parallel for
        ForallDevice(j, gpuErrchk(cudaMemcpy(&dev[j]->dev_is_frequent[is_frequent_index], &is_frequent[is_frequent_index], is_frequent_size,
                                             cudaMemcpyHostToDevice)););
...
int main(int argc, char *argv[]) {
    ...
    assert(devices_use <= total_device_count);
    omp_set_num_threads(devices_use);
