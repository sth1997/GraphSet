 ...
 *        这里 degree orientation 和 degeneracy orientation 两种方法都写了，用于作比较
 * @todo:
 */
...
void degeneracy_orientation_init(Graph* original_g, Graph* &g) {
  ...
  // 这里用 vector<int> node_with_degree[x] 来装当前度数为 x 的所有节点，做到了线性求 k-core 序
  int* degree;
  int* order;
  int* vector_ptr;
  bool* in_vector;
...
__device__ int popcount(unsigned int x) {
  return bit_table[x & BIT_TABLE_COEF] + bit_table[(x >> 8) & BIT_TABLE_COEF] + 
         bit_table[(x >> 16) & BIT_TABLE_COEF] + bit_table[(x >> 24) & BIT_TABLE_COEF];
}
 ...
 */
__global__ void traverse_on_warp_partition
  (int* n, int* thread_block_num, unsigned int* binary_adj, int64_t* vertex, int64_t* start_ptr, int* k) {
  ...
  if (threadIdx.x == 0) {
    ...
    for (int i = 0; i < WARPS_PER_BLOCK; ++i) {
      ...
      stack_binary_adj_ptr[i] = stack_binary_adj_pool + i * THREADS_PER_WARP * MAX_DEPTH;
    }
  }
  __syncthreads();
  ...
  for (int64_t o = (int64_t)blockIdx.x * WARPS_PER_BLOCK + warpidx; o < vertex[*n]; o += (int64_t)thread_block_num * WARPS_PER_BLOCK) {
    int l = 0, r = *n - 1;
    while (l != r) {
      int middle = (l + r >> 1) + 1;
      if (vertex[middle] <= o) {
    ...
    while (~top) {
      if (top + 3 == *k) {
        if (is_main_thread) {
          // sum += popcount(stack_binary_adj + (*partition_num) * top, partition_num);
        ...
        while (stack_vertex[top + 1] < size) {
          ...
          if (!(binary_adj[start_ptr[u] + stack_vertex[top] * partition_num + partitionidx] >> bitidx)) {
            stack_vertex[top + 1] += 32 - bitidx;
            bitidx = 0;
            ++partitionidx;
            continue;
          }
          // if ((stack_binary_adj[top * (*partition_num) + partitionidx] >> bitidx) & 1) {
          if ((binary_adj[start_ptr[u] + stack_vertex[top] * partition_num + partitionidx] >> bitidx) & 1) {
            ++top;
            int i = threadIdx.x % THREADS_PER_WARP;
            if (i < partition_num) {
              stack_binary_adj[top * partition_num + i] =
                stack_binary_adj[(top - 1) * partition_num + i] &
                binary_adj[start_ptr[u] + stack_vertex[top] * partition_num + i];
            }
            // __syncwarp();
...
__global__ void traverse(unsigned int* binary_adj, int64_t* vertex, int64_t* start_ptr, int* k) {
...
void k_clique_counting(Graph* g, int k) {
  ...
  unsigned int* binary_adj = (unsigned int*)malloc(start_ptr[n] * sizeof(unsigned int));
    ...
    for (int64_t i = g->vertex[u]; i < g->vertex[u + 1]; ++i) {
      ...
      while (p < g->vertex[u + 1]) {
        ...
        if (q < g->vertex[v + 1] && g->edge[p] == g->edge[q]) {
          binary_adj[start_ptr[u] + partition_num * (i - g->vertex[u]) + partitionidx] |= 1 << bitidx;
        }
        ++p;
        if (++bitidx == 32) {
          bitidx = 0;
  ...
  int thread_block_num = (int)std::min((long long)MAX_THREAD_BLOCK, (m * THREADS_PER_WARP - 1) / THREADS_PER_BLOCK + 1);
  ...
  cudaMemcpy(gpu_binary_adj, binary_adj, start_ptr[n] * sizeof(unsigned int), cudaMemcpyHostToDevice);
  cudaMemcpy(gpu_vertex, g->vertex, (n + 1) * sizeof(int64_t), cudaMemcpyHostToDevice);
  cudaMemcpy(gpu_start_ptr, start_ptr, (n + 1) * sizeof(int64_t), cudaMemcpyHostToDevice);
  cudaMemcpy(gpu_k, &k, sizeof(int), cudaMemcpyHostToDevice);
  cudaMemcpy(gpu_n, &n, sizeof(int), cudaMemcpyHostToDevice);
  cudaMemcpy(gpu_thread_block_num, &thread_block_num, sizeof(int), cudaMemcpyHostToDevice);
  ...
  traverse<<<n, THREADS_PER_BLOCK>>>(gpu_binary_adj, gpu_vertex, gpu_start_ptr, gpu_k);
  // traverse_on_warp_partition<<<thread_block_num, THREADS_PER_BLOCK>>>
  //   (gpu_n, gpu_thread_block_num, gpu_binary_adj, gpu_vertex, gpu_start_ptr, gpu_k);
...
int main() {
  ...
  original_g->vertex = (int64_t*)malloc((original_g->v_cnt + 1) * sizeof(int64_t));
  for (long long i = 0; i < original_g->e_cnt; ++i) {
