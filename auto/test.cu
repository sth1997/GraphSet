...
__global__ void gpu_pattern_matching(uint32_t edge_num, uint32_t buffer_size, uint32_t *edge_from, uint32_t *edge, uint32_t *vertex, uint32_t *tmp, const GPUSchedule* schedule) {
    ...
    while (true) {
        ...
        for (int i_depth2 = 0; i_depth2 < loop_size_depth2; ++i_depth2) {
            ...
            if (threadIdx.x % THREADS_PER_WARP == 0)
                vertex_set[3].init(r_depth2 - l_depth2, &edge[l_depth2]);
            __threadfence_block();
            ...
            for (int i_depth3 = 0; i_depth3 < loop_size_depth3; ++i_depth3) {
                ...
                intersection2(vertex_set[4].get_data_ptr(), vertex_set[3].get_data_ptr(), &edge[l_depth3], vertex_set[3].get_size(), r_depth3 - l_depth3, &vertex_set[4].size);
                ...
                if (lid == 0) {
                    ...
                    p[2] = v2;
                    p[3] = v3;
                    subtraction_set.set_size(4);
                }
                __threadfence_block();

                int ans0 = unordered_subtraction_size(vertex_set[4], subtraction_set);
                long long val;
                val = ans0;
                val = val * ans0;
                sum += val * 1;
