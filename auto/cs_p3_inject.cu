__global__ void gpu_pattern_matching(uint32_t edge_num, uint32_t buffer_size, uint32_t *edge_from, uint32_t *edge, uint32_t *vertex, uint32_t *tmp, const GPUSchedule* schedule) {
    ...
    while (true) {
        ...
        for (int i_depth2 = 0; i_depth2 < loop_size_depth2; ++i_depth2) {
            ...
            intersection2(vertex_set[2].get_data_ptr(), vertex_set[0].get_data_ptr(), &edge[l_depth2], vertex_set[0].get_size(), r_depth2 - l_depth2, &vertex_set[2].size);
            if (vertex_set[2].get_size() == 1) continue;
            ...
            for (int i_depth3 = 0; i_depth3 < loop_size_depth3; ++i_depth3) {
                ...
                intersection2(vertex_set[3].get_data_ptr(), vertex_set[2].get_data_ptr(), &edge[l_depth3], vertex_set[2].get_size(), r_depth3 - l_depth3, &vertex_set[3].size);
                if (vertex_set[3].get_size() == 0) continue;
                
                v3 = v_depth3; // subtraction_set.push_back(v3);

                int loop_size_depth4 = vertex_set[3].get_size();
                uint32_t* loop_data_ptr_depth4 = vertex_set[3].get_data_ptr();
                for (int i_depth4 = 0; i_depth4 < loop_size_depth4; ++i_depth4) {
                    ...
                    intersection2(vertex_set[4].get_data_ptr(), vertex_set[2].get_data_ptr(), &edge[l_depth4], vertex_set[2].get_size(), r_depth4 - l_depth4, &vertex_set[4].size);
                    if (vertex_set[4].get_size() == 1) continue;
                    
                    v4 = v_depth4; // subtraction_set.push_back(v4);

                    int loop_size_depth5 = vertex_set[4].get_size();
                    uint32_t* loop_data_ptr_depth5 = vertex_set[4].get_data_ptr();
                    uint32_t min_vertex_depth5 = 0xffffffff;
                    min_vertex_depth5 = min(min_vertex_depth5, v3);
                    for (int i_depth5 = 0; i_depth5 < loop_size_depth5; ++i_depth5) {
                        uint32_t v_depth5 = loop_data_ptr_depth5[i_depth5];
                        if (min_vertex_depth5 <= v_depth5) break;
