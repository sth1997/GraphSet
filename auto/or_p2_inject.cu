__global__ void pattern_matching_kernel(uint32_t edge_num, uint32_t buffer_size, uint32_t *edge_from, uint32_t *edge, uint32_t *vertex, uint32_t *tmp) {
    ...
    while (true) {
        ...
        for (int i_depth2 = 0; i_depth2 < loop_size_depth2; ++i_depth2) {
            ...
            int ans0 = vertex_set[3].get_size() - 1;
            int ans1 = vertex_set[1].get_size() - 1;
            int ans2 = vertex_set[4].get_size() - 1;
            int ans3 = vertex_set[5].get_size() - 0;
            long long val;
            val = ans0;
            val = val * ans1;
            val = val * ans2;
...
unsigned long long do_pattern_matching(Graph* g,
    ...
    cudaOccupancyMaxActiveBlocksPerMultiprocessor(&max_active_blocks_per_sm,
        pattern_matching_kernel, THREADS_PER_BLOCK, 992);
    int nr_blocks = 1024;
    ...
    size_t size_tmp = VertexSet::max_intersection_size * sizeof(uint32_t) * nr_total_warps * 6;
    ...
    pattern_matching_kernel<<<nr_blocks, THREADS_PER_BLOCK, 992>>>
