...
//前n个点组成的子图的边数
int get_schedule_edge_num(const Schedule& p, int n = -1)
{
    if (n == -1)
        n = p.get_size();
    int edge_num = 0;
...
bool check_isomorphism_dfs(int schedule_id, const Schedule& s, const Schedule& s0, std::vector<int>& mappings_vec, int size, int depth, int* mapping, const int* connected_v = NULL) {
...
bool check_isomorphism(int schedule_id, const Schedule& s, const Schedule& s0, std::vector<int>& mappings_vec, int size, const int* connected_v = NULL) {
...
void generate_mapping_plans(const Schedule& s, const Schedule* schedules, std::vector<int>& mappings_vec, int* mapping_start_idx, int& mapping_start_idx_pos, int s_id) {
    for (int depth = 1; depth < s.get_size(); ++depth) {
        ...
        for (int s0_id = 0; s0_id < s_id; ++s0_id) { //已经按照边数排好序了，所以s的子图一定只在s_id之前
            const Schedule& s0 = schedules[s0_id];
...
bool LabeledGraph::get_support_pattern_matching_aggressive_func(const Schedule& schedule, const char* p_label, VertexSet* vertex_set, VertexSet& subtraction_set, std::vector<std::unordered_set<int> >& fsm_set, int depth) const {
...
long long LabeledGraph::get_support_pattern_matching(VertexSet* vertex_set, VertexSet& subtraction_set, const Schedule& schedule, const char* p_label, std::vector<std::unordered_set<int> >& fsm_set, long long min_support) const {
    /*
...
void LabeledGraph::get_support_pattern_matching_vertex(int vertex, VertexSet* vertex_set, VertexSet& subtraction_set, const Schedule& schedule, const char* p_label, std::vector<std::unordered_set<int> >& fsm_set, int min_support) const {
    /*
    //一个点的pattern在omp之前被特殊处理了
    if (schedule.get_size() == 1) {
        long long support = 0;
...
int LabeledGraph::fsm_vertex(int job_id, const Schedule &schedule, const char *all_p_label, std::vector<std::vector<int> > &automorphisms, unsigned int* is_frequent, unsigned int& pattern_is_frequent_index, int max_edge, int min_support, int thread_count) const {
    ...
    std::vector<std::unordered_set<int> > fsm_set;
    fsm_set.clear();
    for (int j = 0; j < max_edge + 1; ++j) { //至多max_edge + 1个点
        fsm_set.push_back(std::unordered_set<int>());
        fsm_set.back().clear();
    }
    #pragma omp parallel num_threads(thread_count)
    {
        VertexSet* vertex_set = new VertexSet[schedule.get_total_prefix_num()];
        std::vector<std::unordered_set<int> > local_fsm_set;
        local_fsm_set.clear();
        for (int j = 0; j < max_edge + 1; ++j) { //至多max_edge + 1个点
            local_fsm_set.push_back(std::unordered_set<int>());
