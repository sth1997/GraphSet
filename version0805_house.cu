...
__device__ void intersection(uint32_t *tmp, uint32_t *lbases, uint32_t *rbases, uint32_t ln, uint32_t rn, uint32_t* p_tmp_size) {
    __shared__ uint32_t lblock[32];
    __shared__ uint32_t rblock[32];

    __shared__ uint32_t cur_thread;

    uint32_t i = 0, j = 0;
    uint32_t lsize = 32, rsize = 32;
...
void gpu_pattern_matching(Graph *g) {
    ...
    __dfs<<<numBlocks, 32>>>(edge_num, buffer_size, dev_edge_from, dev_edge, dev_vertex, dev_tmp);
